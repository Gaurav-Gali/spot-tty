...
impl CoverImage {
    ...
    pub fn render(
        ...
        match protocol {
            ...
            ImageProtocol::HalfBlock => self.render_halfblock(frame, area, cache),
        }
    }
    ...
    fn render_halfblock(&self, frame: &mut Frame, area: Rect, cache: &mut RenderCache) {
        let w = area.width;
        let h = area.height;
        let key = (self.kitty_id, w, h);

        // Resize once into RenderCache — free on subsequent frames
        if !cache.halfblock.contains_key(&key) {
            let resized = self
                .decoded
                .resize_exact(w as u32, (h * 2) as u32, FilterType::Lanczos3);
            cache.halfblock.insert(key, resized.to_rgba8().into_raw());
        }
        let pixels = cache.halfblock.get(&key).unwrap();
        let stride = (w * 4) as usize;

        let buf = frame.buffer_mut();
        for row in 0..h {
            for col in 0..w {
                let top_off = (row as usize * 2) * stride + col as usize * 4;
                let bot_off = (row as usize * 2 + 1) * stride + col as usize * 4;
